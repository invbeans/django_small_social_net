...
def loginpage(request):
    ...
    try:
        ...
        request.session[
            'user_fathers_name'] = current_registered_user.fathers_name
...
def current_post(request, param, post_id):
    ...
    if (is_int):
        ...
        current_registered_user = RegisteredUser.objects.get(id=param)
        if (current_registered_user.id == user_id): is_self = True
    else:
        user_custom_url = request.session['user_custom_url']
        current_post = UserPost.objects.get(id=post_id)
        current_registered_user = RegisteredUser.objects.get(custom_url=param)
...
class UserPostCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        if (type(self.kwargs['param']) == int):
            self.user = get_object_or_404(RegisteredUser,
                                          id=self.kwargs['param'])
        else:
            self.user = get_object_or_404(RegisteredUser,
                                          custom_url=self.kwargs['param'])
...
class RegisteredUserUpdateView(UpdateView):
    ...
    fields = ('login', 'password', 'name', 'surname', 'fathers_name', 'age',
              'email', 'custom_url')
    ...
    def get_object(self):
        ...
        self.request.session[
            'user_custom_url'] = current_registered_user.custom_url
